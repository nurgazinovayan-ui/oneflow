...
-- Internal messenger: DMs + group channels, restricted to @mechta.kz accounts plus the app
-- owner's own account (nurgazinov.ayan@gmail.com), which isn't on that domain. No changes to
-- existing tables.
--
...
-- re-checks eligibility against the caller's own verified JWT rather than trusting anything
-- the client claims. Only plain SELECTs are exposed to authenticated clients here, the same
-- way admin_messages/subscriptions are read directly via REST elsewhere in this app.
create extension if not exists pgcrypto with schema extensions;

-- Single source of truth for "who may use the messenger", reused by every CHECK constraint and
-- RLS policy below (and duplicated -- deliberately, see the Edge Functions' own file header
-- comments -- as a small inline check in each Edge Function, which cannot call into Postgres
-- functions before it has authenticated).
create or replace function public.is_messenger_allowed_email(p_email text) returns boolean
language sql immutable as $$
  select coalesce(p_email, '') ~* '^[^@\s]+@mechta\.kz$' or lower(coalesce(p_email, '')) = 'nurgazinov.ayan@gmail.com';
$$;

create table if not exists public.messenger_profiles (
  email text primary key check (public.is_messenger_allowed_email(email)),
  display_name text not null default '' check (length(display_name) <= 120),
  status text not null default 'idle' check (status in ('idle', 'generating', 'copywriting', 'evaluating')),
  last_seen_at timestamptz not null default now()
);
alter table public.messenger_profiles add column if not exists status text not null default 'idle';
alter table public.messenger_profiles drop constraint if exists messenger_profiles_status_check;
alter table public.messenger_profiles add constraint messenger_profiles_status_check
  check (status in ('idle', 'generating', 'copywriting', 'evaluating'));

create table if not exists public.messenger_channels (
  id uuid primary key default gen_random_uuid(),
  kind text not null check (kind in ('dm', 'group')),
  title text not null default '' check (length(title) <= 120),
  created_by text not null check (public.is_messenger_allowed_email(created_by)),
...
create table if not exists public.messenger_members (
  ...
  email text not null check (public.is_messenger_allowed_email(email)),
...
create table if not exists public.messenger_messages (
  id uuid primary key default gen_random_uuid(),
  channel_id uuid not null references public.messenger_channels(id) on delete cascade,
  sender_email text not null check (public.is_messenger_allowed_email(sender_email)),
...
create policy "Mechta roster read" on public.messenger_profiles for select to authenticated
  using (public.is_messenger_allowed_email(lower(coalesce(auth.jwt() ->> 'email', ''))));
...
-- write goes through a service-role Edge Function, which re-validates eligibility and channel
-- membership against the caller's verified JWT before touching these tables.
